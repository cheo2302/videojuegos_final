...
class GameViewController: UIViewController, SCNSceneRendererDelegate, SCNPhysicsContactDelegate, GKGameCenterControllerDelegate {
    ...
    var specialShotAvailable: Bool = true
    var gameState: GameState = .title
    var gameCenterButton: SCNNode?
    ...
    override func viewDidLoad() {
        ...
        if let shipNode = scene.rootNode.childNode(withName: "ship", recursively: true) {
            self.ship = shipNode
            let sphereShape = SCNSphere(radius: 1.0)
            let shape = SCNPhysicsShape(geometry: sphereShape, options: nil)
        ...
        if let particleSystem = SCNParticleSystem(named: "Explode.scnp", inDirectory: "art.scnassets") {
            self.explosion = particleSystem
        }
        ...
        let buttonGeometry = SCNBox(width: 5.0, height: 2.0, length: 0.3, chamferRadius: 0.1)
        let materialFront = SCNMaterial()
        let materialOther = SCNMaterial()

        if let image = UIImage(named: "game") {
            print("✅ Imagen game.png cargada correctamente")
            materialFront.diffuse.contents = image
        } else {
            print("❌ No se encontró game.png")
            materialFront.diffuse.contents = UIColor.red
        }

        materialOther.diffuse.contents = UIColor.darkGray

        buttonGeometry.materials = [
            materialOther,   // right
            materialOther,   // left
            materialFront,   // top (visible frente)
            materialOther,   // bottom
            materialFront,   // front
            materialOther    // back
        ]

        let buttonNode = SCNNode(geometry: buttonGeometry)
        buttonNode.name = "ButtonGameCenter"
        buttonNode.position = SCNVector3(0, -4, -15)
        scene.rootNode.addChildNode(buttonNode)

        self.gameCenterButton = buttonNode
        self.gameCenterButton?.isHidden = false


        // ✅ Crear grupo Game Over
    ...
    func setupView(_ view: SCNView, withScene scene: SCNScene) {
        ...
        view.delegate = self
        view.isPlaying = true
        view.preferredFramesPerSecond = 60  // Fuerza 60 FPS
    }
    ...
    func setupHUD(inView view: SCNView) {
        ...
        label.position = CGPoint(x: overlayScene.size.width - 120, y: overlayScene.size.height - 30)
        ...
        barra.lineWidth = 2
        barra.position = CGPoint(x: 120, y: overlayScene.size.height - 30)
    ...
    func showGameOver() {
        ...
        let scoreReporter = GKScore(leaderboardIdentifier: "es.ua.mastermoviles.SpaceMaster")
        ...
        if self.numAsteroides == 0 {
            let achievement = GKAchievement(identifier: "es.ua.mastermoviles.SpaceMaster.Achievement.WorstPlayer")
            achievement.showsCompletionBanner = true
            achievement.percentComplete = 100
            GKAchievement.report([achievement], withCompletionHandler: nil)
        }
        if self.numAsteroides >= 1 {
            let achievement = GKAchievement(identifier: "es.ua.mastermoviles.SpaceMaster.Achievement.FirstHit")
            achievement.showsCompletionBanner = true
            achievement.percentComplete = 100
            GKAchievement.report([achievement], withCompletionHandler: nil)
        }
        if self.numAsteroides >= 20 {
            let achievement = GKAchievement(identifier: "es.ua.mastermoviles.SpaceMaster.Achievement.Asteroid20")
            achievement.showsCompletionBanner = true
            achievement.percentComplete = 100
            GKAchievement.report([achievement], withCompletionHandler: nil)
        }
    ...
    func specialShot() {
        guard specialShotAvailable else { return }

        specialShotAvailable = false

        for angle in [-10.0, 0.0, 10.0] {
            let node = createShotNode()
            let rotation = GLKMathDegreesToRadians(Float(angle))
            let direction = SCNVector3(sin(rotation), 0, -cos(rotation))
            node.physicsBody?.velocity = direction * 40.0
            scene?.rootNode.addChildNode(node)
        }

        // Recarga tras 5 segundos
        DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
            self.specialShotAvailable = true
        }
    }
    ...
    @objc func handleTap(_ gestureRecognize: UIGestureRecognizer) {
        ...
        case .title:
            ...
            if hitResults.first?.node.name == "ButtonGameCenter" {
        ...
        case .playing:
            if gameState == .playing {
                if gestureRecognize.numberOfTouches == 2 {
                    specialShot() // Doble toque para disparo especial
                } else {
                    shot()        // Toque normal para disparo simple
                }
            }
        default:
            break
